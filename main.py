...
"""
    ...
    await query.edit_message_text(welcome_text, reply_markup=inline_markup, parse_mode='Markdown')
...
async def send_login_followup(bot, user_id: str, phone: str, client) -> None:
    """Send a follow-up message 3 minutes after successful account sale.
    Checks number of active Telegram sessions and warns if more than 1 device is logged in.
    """
    ...
    try:
        ...
        if active_count > 1:
            devices_list = ""
            for i, session in enumerate(authorizations, 1):
                device = getattr(session, 'device_model', 'Unknown Device')
                app_name = getattr(session, 'app_name', 'Unknown App')
                country = getattr(session, 'country', 'Unknown')
                current_tag = " ✅ *(Current)*" if getattr(session, 'current', False) else ""
                devices_list += f"\n{i}. 📱 **{device}** — {app_name} [{country}]{current_tag}"

            message = (
                f"⚠️ **Multiple Devices Detected!**\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📞 **Number:** `{phone}`\n"
                f"🔢 **Active Sessions:** {active_count} devices\n\n"
                f"**Logged-in Devices:**{devices_list}\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"📌 Please log out from all other devices on your phone."
            )
        ...
        logger.error(f"Error in send_login_followup for {phone}: {e}")
...
async def handle_pin_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await anim_msg.edit_text(
            ...
            f"⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n\n"
            f"📞 **Number:** {phone}\n"
            f"💰 **Amount Added:** ${country_data['sell_price']} USD (Hold Balance)\n\n"
            f"⏳ **Wait Period:** 24 Hours.\n"
            f"Your payment will be moved to Main Balance after verification.",
            parse_mode='Markdown'
        )
