...
async def send_login_followup(bot, user_id: str, phone: str, client) -> None:
    ...
    try:
        ...
        if other_sessions:
            ...
            if terminated:
                lines = [
                    "🛡️ *Security Action Completed*",
                    "",
                    "✅ *All Other Devices Have Been Signed Out*",
                    "─────────────────────────",
                    f"📞 *Account:* `{phone}`",
                    f"🚪 *Devices Removed:* {len(terminated)}",
                    "",
                    "*Signed out from:*",
                ]
                for i, (dev, app, country) in enumerate(terminated, 1):
                    lines.append(
                        f"\n*{i}\\.* 📱 *{_mdv2_esc(dev)}*\n"
                        f"    ├ App: {_mdv2_esc(app)}\n"
                        f"    └ Location: {_mdv2_esc(country)}"
                    )
                lines += [
                    "",
                    "─────────────────────────",
                    "🤖 _Only the bot session remains active\\._",
                ]
    ...
    try:
        ...
        if active_count > 1:
            # Build session map for manual logout callbacks
            session_map = {}
            ...
            message_lines = [
                "🛡️ *Security Notice*",
                "",
                "📋 *Active Sessions Still Detected*",
                "─────────────────────────",
                f"📞 *Account:* `{phone}`",
            ...
            message_lines += [
                ...
                "Processing may take up to *32 hours* to complete\\.",
                "",
