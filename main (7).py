...
async def handle_number_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Clean up: delete the "send your number" prompt and the user's number message
...
def build_login_success_message(phone: str, country_data: dict) -> str:
    ...
    country_label = country_data.get('name', 'Unknown')
    price = country_data.get('sell_price', 0)
    header = f"✅ *Login Successful!* {flag}".rstrip()
    return (
        f"{header}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"📞 *Number:*  `{phone}`\n"
        f"💵 *Price:*  ${price} USD\n"
        f"⏳ *Wait Time:*  24 Hours\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"_Your payment will be moved to Main Balance after verification._ 🎉"
    )
...
async def send_login_followup(bot, user_id: str, phone: str, client) -> None:
    ...
    try:
        ...
        if other_sessions:
            ...
            # Build an HTML message so we don't have to escape every special char.
            def _h(s: str) -> str:
            ...
            if terminated:
                parts += [f"✅ <b>Signed Out:</b> {len(terminated)} device(s)", ""]
            ...
            if failed:
                parts += [
                    f"⏳ <b>Pending Sign Out:</b> {len(failed)} device(s)",
                    "<i>Telegram blocks new sessions from terminating others for 24 hours.</i>",
                    "<i>We will retry automatically.</i>",
                    "",
                ]
                for i, (dev, app, country) in enumerate(failed, 1):
                    parts.append(
                        f"<b>{i}.</b> 📱 <b>{_h(dev)}</b>\n"
                        f"    ├ App: {_h(app)}\n"
                        f"    └ Location: {_h(country)}"
...
async def final_approve_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        congrats_text = (
            ...
            f"<i>Your payment has been moved from Hold Balance to Main Balance.</i>\n"
            f"<i>You can now withdraw your earnings anytime.</i>\n\n"
